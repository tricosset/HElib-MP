...
// Choose random c0,c1 such that c0+s*c1 = p*e for a short e
void RLWE(DoubleCRT& c0,DoubleCRT& c1, const DoubleCRT &s, ZZ& p, ZZ* prgSeed)
{
  assert (p>0); // Can be used with p=1, but we always use with p>=2
...
// same as above, but assumes that c1 is already chosen by the caller
void RLWE1(DoubleCRT& c0, const DoubleCRT& c1, const DoubleCRT &s, ZZ& p)
{
  assert (p>0); // Can be used with p=1, but we always use with p>=2
			...
			bool highNoise) const
{
  ...
  if (ptxtSpace != pubEncrKey.ptxtSpace) { // plaintext-space mistamtch
    ptxtSpace = GCD(ptxtSpace, ptxtSpace, pubEncrKey.ptxtSpace);
...
// returned value is the plaintext-space for that ciphertext. When called
// with highNoise=true, returns a ciphertext with noise level~q/8.
ZZ FHEPubKey::Encrypt(Ctxt &ctxt, const ZZX& ptxt, ZZ ptxtSpace,
			bool highNoise) const
{
  FHE_TIMER_START;
  assert(this == &ctxt.pubKey);
  ...
  if (ptxtSpace != pubEncrKey.ptxtSpace) { // plaintext-space mistamtch
	GCD(ptxtSpace, ptxtSpace, pubEncrKey.ptxtSpace);
	if (ptxtSpace <= 1) Error("Plaintext-space mismatch on encryption");
  }
...
#ifdef BIG_P
void FHEPubKey::NFLlib2HElib(Ctxt &ctxt, const char* file_c0, const char* file_c1, ZZ ptxtSpace) const
{
#ifdef VERBOSE
	std::cout << "FHESecKey::Encrypt" << std::endl;
#endif
  FHE_TIMER_START;
  ...
  ctxt.ptxtSpace = ptxtSpace;
  xdouble phim = to_xdouble(context.zMStar.getPhiM());
  ctxt.noiseVar = context.stdev*context.stdev * phim * to_xdouble(ptxtSpace)*to_xdouble(ptxtSpace);

#ifdef VERBOSE
std::cout << "\tctxt.noiseVar = context.stdev*context.stdev * phim * to_xdouble(ptxtSpace)*to_xdouble(ptxtSpace);" << std::endl
		  << "\tcontext.stdev: " << context.stdev << std::endl
		  << "\tphim: " << phim << std::endl
		  << "\tto_xdouble(ptxtSpace): " << to_xdouble(ptxtSpace) << std::endl
          << "\tctxt.noiseVar: " << ctxt.noiseVar << " (" << log(ctxt.noiseVar)/log(2)/2 << ")" << std::endl;
#endif
  // Make parts[0],parts[1] point to (1,s)
  //  ctxt.parts[0].skHandle.setOne();
  //  ctxt.parts[1].skHandle.setBase(skIdx);
  ifstream file0(file_c0, ios::in);
  ifstream file1(file_c1, ios::in);
  ZZX c0, c1;
  file0 >> c0;
  file1 >> c1;
  file0.close();
...
#else
  assert( p == pk.getContext().ModulusP() );
#endif
#ifndef BIG_P
...
// It is assumed that the context already contains all parameters.
long FHESecKey::ImportSecKey(const DoubleCRT& sKey, long Hwt,
			     ZZ& ptxtSpace, bool onlyLinear)
{
  if (sKeys.empty()) { // 1st secret-key, generate corresponding public key
	    if (ptxtSpace<2)
	      ptxtSpace = context.ModulusP(); // default plaintext space is p^r
    ...
    xdouble phim = to_xdouble(context.zMStar.getPhiM());
    pubEncrKey.noiseVar = context.stdev * context.stdev
      * phim * to_xdouble(ptxtSpace) * to_xdouble(ptxtSpace);
  }
  skHwts.push_back(Hwt); // record the Hamming weight of the new secret-key
  sKeys.push_back(sKey); // add to the list of secret keys
  long keyID = sKeys.size()-1; // not thread-safe?
			...
			ZZX& f) const // plaintext before modular reduction
{
  ...
  if (ciphertxt.ptxtSpace>2) { // if p>2, multiply by Q^{-1} mod p
    ZZ qModP;
    rem(qModP, context.productOfPrimes(ciphertxt.getPrimeSet()),
		     ciphertxt.ptxtSpace);
    if (qModP != 1) {
      InvMod(qModP, qModP, ciphertxt.ptxtSpace);
...
// Decryption
long FHESecKey::Noise(ZZX& plaintxt, const Ctxt &ciphertxt) const
{
  ZZX f;
  return Noise(plaintxt, ciphertxt, f);
}
long FHESecKey::Noise(ZZX& plaintxt, const Ctxt &ciphertxt,
			ZZX& f) const // plaintext before modular reduction
{
	ZZX ns;
#ifdef DEBUG_PRINTOUT
...
//    if (qModP != 1) {
//      InvMod(qModP, qModP, ciphertxt.ptxtSpace);
//      MulMod(ns, ns, qModP, ciphertxt.ptxtSpace);
//    }
//  }
  ns -= plaintxt;
  return MaxBits(ns);
...
// Encryption using the secret key, this is useful, e.g., to put an
// encryption of the secret key into the public key.
ZZ FHESecKey::Encrypt(Ctxt &ctxt, const ZZX& ptxt,
			ZZ ptxtSpace, long skIdx) const
{
#ifdef VERBOSE
	std::cout << "FHESecKey::Encrypt" << std::endl;
#endif
...
#ifdef VERBOSE
  ZZX p2(0);
  std::cout << " \033[31m/!\\ Noise before addConstant: " << this->Noise(p2, ctxt) << "\033[0m" << std::endl;
  p2 = ptxt;
  // add in the plaintext
  ctxt.addConstant(ptxt);
  // ctxt.addConstant(ptxt, to_double(coeffsL2Norm(ptxt))*to_double(coeffsL2Norm(ptxt)), *this, p2);
  std::cout << " \033[31m/!\\ Noise after addConstant: " << this->Noise(p2, ctxt) << "\033[0m" << std::endl;
#else
  ctxt.addConstant(ptxt);
  // ctxt.addConstant(ptxt, to_double(coeffsL2Norm(ptxt))*to_double(coeffsL2Norm(ptxt)));
#endif
