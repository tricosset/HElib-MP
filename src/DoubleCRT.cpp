...
#else
void DoubleCRT::scaleDownToSet(const IndexSet& s, ZZ ptxtSpace)
{
  assert(ptxtSpace >= 2);
  ...
  // by ptxtSpace.
	...
	ZZ p_over_2 = ptxtSpace/2;
	ZZ prodInv;
	InvMod(prodInv, diffProd % ptxtSpace, ptxtSpace);
	for (long i = 0; i < delta_len; i++) {
      ZZ delta_i_modP;
    ...
    ZZ prodInv;
    InvMod(prodInv, diffProd % ptxtSpace, ptxtSpace);
    long nbits = NumBits(ptxtSpace)+1;
    ZZ word = power2_ZZ(nbits);
    ZZ precon = (word*prodInv/ptxtSpace);
    ZZ delta_i_modP;
    ZZ quo;
    ...
    for (long i=0; i<delta_len; i++) {
      rem(delta_i_modP, delta.rep[i], ptxtSpace);
      if (delta_i_modP != 0) {
        RightShift(quo,precon*delta_i_modP,nbits);
        quo *= ptxtSpace;
        delta_i_modP *= prodInv;
        delta_i_modP -= quo;
        if (delta_i_modP > ptxtSpace) {
          delta_i_modP -= ptxtSpace;
        }
        if (delta_i_modP > p_over_2) {
          delta_i_modP -= ptxtSpace;
        }
        delta_i_modP *= diffProd;
        delta.rep[i] -= delta_i_modP;
      }
    }
  }
#endif
