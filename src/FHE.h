...
#else
  explicit
  KeySwitch(long sPow=0, long xPow=0, long fromID=0, long toID=0, ZZ p = to_ZZ(0)):
    fromKey(sPow,xPow,fromID),toKeyID(toID),ptxtSpace(p) {}
  explicit
  KeySwitch(const SKHandle& _fromKey, long fromID=0, long toID=0, ZZ p = to_ZZ(0)):
    fromKey(_fromKey),toKeyID(toID),ptxtSpace(p) {}
#endif
...
public:
  FHEPubKey(): // this constructor thorws run-time error if activeContext=NULL
    context(*activeContext), pubEncrKey(*this), recryptEkey(*this, to_ZZ(0))
      { recryptKeyID=-1; }

  explicit
  FHEPubKey(const FHEcontext& _context): 
    context(_context), pubEncrKey(*this), recryptEkey(*this, to_ZZ(0))
      { recryptKeyID=-1; }
...
#else
  ZZ getPtxtSpace() const { return pubEncrKey.ptxtSpace; }
#endif
  ...
  //! The returned value is the plaintext-space for that ciphertext. When
  //! called with highNoise=true, returns a ciphertext with noise level~q/8.
  ZZ Encrypt(Ctxt &ciphertxt, const ZZX& plaintxt, ZZ ptxtSpace = to_ZZ(0),
	       bool highNoise=false) const;

  void NFLlib2HElib(Ctxt &ciphertxt, const char* file_c0, const char* file_c1, ZZ ptxtSpace = to_ZZ(0)) const;
#endif
...
public:
  ...
  //! It is assumed that the context already contains all parameters.
  long ImportSecKey(const DoubleCRT& sKey, long hwt,
		    ZZ& ptxtSpace, bool onlyLinear=false);
  ...
  //! Key generation: This procedure generates a single secret key,
  //! pushes it onto the sKeys list using ImportSecKey from above.
  long GenSecKey(long hwt, ZZ& ptxtSpace, bool onlyLinear=false)
  { DoubleCRT newSk(context); // defined relative to all primes, special or not
    newSk.sampleHWt(hwt);     // samle a Hamming-weight-hwt polynomial
    return ImportSecKey(newSk, hwt, ptxtSpace, onlyLinear);
  }
  ...
  //! by context.mod2r.
  void GenKeySWmatrix(long fromSPower, long fromXPower, long fromKeyIdx=0,
		      long toKeyIdx=0, ZZ ptxtSpace = to_ZZ(0));
#endif

  // Decryption
  void Decrypt(ZZX& plaintxt, const Ctxt &ciphertxt) const;
  long Noise(ZZX& plaintxt, const Ctxt &ciphertxt) const;
  ...
  long Noise(ZZX& plaintxt, const Ctxt &ciphertxt, ZZX& f) const;
...
#else
  //! @brief Symmetric encryption using the secret key.
  ZZ Encrypt(Ctxt &ctxt, const ZZX& ptxt,
	       ZZ ptxtSpace, long skIdx=0) const;
#endif
...
//! Choose random c0,c1 such that c0+s*c1 = p*e for a short e
void RLWE(DoubleCRT& c0, DoubleCRT& c1, const DoubleCRT &s, ZZ& p,
	  ZZ* prgSeed=NULL);
