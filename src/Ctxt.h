...
class Ctxt {
...
#else
  ZZ ptxtSpace;      // plaintext space for this ciphertext (either p or p^r)
#endif
  ...
  void addPart(const DoubleCRT& part, const SKHandle& handle, const FHESecKey& secretKey, ZZX& m)
  { addPart(part, handle, false, false, secretKey, m); }
...
public:
  ...
  long uselessPrimes(FHESecKey& secretKey, ZZX& m);
...
#else
  Ctxt(const FHEPubKey& newPubKey, ZZ newPtxtSpace = to_ZZ(0)); // constructor
  Ctxt(const FHEPubKey& newPubKey, const char* file_c0, const char* file_c1, ZZ newPtxtSpace = to_ZZ(0)); // constructor
#endif
 ...
 // Add/subtract aonther ciphertext
  ...
  void addConstant(const DoubleCRT& dcrt, double size, const FHESecKey& secretKey, ZZX& m);
  void addConstant(const ZZX& poly, double size, const FHESecKey& secretKey, ZZX& m);
  //{ addConstant(DoubleCRT(poly,context,primeSet),size,secretKey,m); }
  ...
  void multiplyBy(const Ctxt& other, FHESecKey secretKey, ZZX m);
  void multiplyBy0(const Ctxt& other, FHESecKey secretKey, ZZX m);
  ...
  ///@{
  //! Reduce plaintext space to a divisor of the original plaintext space
  void reducePtxtSpace(ZZ newPtxtSpace);
...
#else
  const ZZ getPtxtSpace() const      { return ptxtSpace;}
#endif
